...
class LIFE_Simulator:
    def __init__(self, network_file_name):
        ...
        self.list_S_matrix = self.create_S_matrix(
            net_df=self.network_df,
            meta_lookup=self.meta_lookup,
            sym_array=self.meta_sym_array,
        )
        ...
        for row in edge_columns_df.itertuples():  # iterate through each of the edges
            ...
            for uber in uber_modulators:
                ...
                if uber[-1] == "+":  # check the last term in the entry, enhancer
                    ...
                    index_of_uber_meta = meta_lookup[
                        uber_meta
            ...
            if num_subs > 1 or num_prods > 1:  # hyperedge!
                ...
                for (
                    sub
                ) in substrates:  # get the index for all the substrates in the edge
    ...
    def set_metabolites_to_fix(self, list_of_meta):
        ...
        self.indices_of_fixed_metabolites = self.extract_indices(
            self.metabolites_to_fix, self.meta_lookup
        )
    ...
    def simulate(self, t):
        ...
        solution = odeint(
            metabolites_func_to_int, self.val_array, t, args=(arg_a, arg_b, arg_c)
        )
...
if __name__ == "__main__":
    ...
    sim_obj.save_simulation(filename="temp/high_gba_12h")
